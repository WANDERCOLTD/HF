...
| **What it proves** | 1 fresh learner created + enrolled in IELTS Speaking Practice V1.0 (playbook UUID `eb6bc79e-…`); bootstrap ComposedPrompt persisted via `autoComposeForCaller`; 3 calls run via `sim-drive-call.ts` with `--module=part1,part2,part3` and 5 turns each; between each call, the script polls (3s interval, 90s timeout) for a new `ComposedPrompt` whose `triggerCallId === <just-ended call.id>` AND `status === 'active'` (next call refuses to run until COMPOSE lands); after all 3 calls, snapshots state and asserts: (i) 3 `Call` rows with `endedAt` set, (ii) 3 pipeline-triggered `ComposedPrompt` rows whose `triggerCallId`s match the 3 calls in order, (iii) per-module `CallerModuleProgress.mastery > 0` AND `status === 'IN_PROGRESS'` (the #950 invariant), (iv) at least one `CallerAttribute lo_mastery:*` row per module that received scorable turns. |
