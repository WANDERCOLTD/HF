...
| `db-registry-parity.md` | `tests/lib/registry/parameter-domain-group-taxonomy.test.ts` (#1948 — JSON source) + `eslint-rules/no-bare-parameter-write.mjs` (#2034 S1 — write chokepoint) + `lib/registry/canonical-domain-group.ts::resolveCanonicalDomainGroup()` runtime helper + planned `tests/lib/registry/parameter-domain-group-db-parity.test.ts` (#2040 S7 — DB ratchet) + planned `prisma/migrations/<ts>_2031_s3c_parameter_domain_group_check/migration.sql` (S3c — CHECK constraint) | ⚠️ PARTIAL (S7 + S3c pending; ratchet covers S1 + JSON-source today, DB CHECK + DB parity test land after S3a/S3b mapping clears incumbent debt) |
