...
| `session-focus-cascade` end-to-end (AGGREGATE Skill scores → CALLER_ATTRIBUTE_NEXT runner → CallerAttribute → COMPOSE transform → learner pin) | `apps/admin/lib/pipeline/aggregate-runner.ts` (CallerTarget.currentScore for skill_* params) | `apps/admin/lib/pipeline/runners/session-focus-policy.ts::runSessionFocusPolicy` (reads CallerTarget, writes `CallerAttribute(scope=specSlug, key=session_focus:next_{moduleSlug})`) → `apps/admin/lib/prompt/composition/transforms/session-focus.ts` (compose-time reader projects to `{directive, pinnedCard}`) → renderer push under `[SESSION FOCUS]` block | ⚠️ PARTIAL | runner unit tests at `apps/admin/tests/lib/pipeline/runners/session-focus-policy.test.ts` (20 tests after #2154 dispatch addition); transform tests at `apps/admin/tests/lib/prompt/composition/transforms/session-focus.test.ts` (11 tests); compose pairs registered in `apps/admin/tests/lib/prompt/composition/coverage-producer-consumer.test.ts` | LOW | Phase A landed by PR #2153 (Phase A) + #2154 (CALLER_ATTRIBUTE_NEXT outputType wiring this row). Phase B (IELTS-P3-FOCUS-001 spec authoring) is epic #2145 S4 — depends on #2137 wiring real Skill scores via IELTS-MEASURE-001. Honest empty-state: writes nothing when no scored CallerTarget rows exist. Distinct from ADAPT (which writes CallerTarget). |
