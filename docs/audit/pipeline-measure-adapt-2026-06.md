...
| **G2** | **CRIT** | M | "[#1142](https://github.com/WANDERCOLTD/HF/issues/1142) Investigate why SKILL-AGG-001 EMA writes never land on `CallerTarget.currentScore`" | all 6 | Add per-call entry/exit logs to `accumulateSkillScores`; assert non-null `currentScore` on at least one IELTS skill param within 1 day of fix. |
