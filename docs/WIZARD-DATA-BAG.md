...
| `welcomeGoals` | experience | boolean | `Playbook.config.welcome.goals.enabled` + `sessionFlow.intake.goals.enabled` (`wizard-tool-executor.ts:44, 66`) | Welcome flow phases | No | n/a | — |
| `welcomeAboutYou` | experience | boolean | `welcome.aboutYou.enabled` + `sessionFlow.intake.aboutYou.enabled` | Welcome flow phases | No | n/a | — |
| `welcomeKnowledgeCheck` | experience | boolean | `welcome.knowledgeCheck.enabled` + `sessionFlow.intake.knowledgeCheck.enabled` | Welcome flow phases | No | n/a | — |
| `welcomeAiIntro` | experience | boolean | `welcome.aiIntroCall.enabled` + `sessionFlow.intake.aiIntroCall.enabled` | Welcome flow phases | No | n/a | — |
