...
**Special case — `progressionMode` (#398).** The AI does NOT silently auto-infer this field. Before `create_course`, rule 5d in `v5-system-prompt.ts` requires a 2-option `show_suggestions` picker; the educator's click triggers an explicit `update_setup({ progressionMode })` call. `setupData.curriculumPath` only determines the default ordering of the picker (`"authored"` → `["Let learners pick (recommended)", …]`; otherwise `["AI directs the sequence", …]`). `courseRefDigest.modulesAuthored` is NOT present in the runtime digest shape and must not be referenced.
