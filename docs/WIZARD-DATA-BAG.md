...
**Special case — `progressionMode` (#398).** The AI does NOT silently auto-infer this field. Before `create_course`, rule 5d in `v5-system-prompt.ts` + a structural BLOCKED directive emitted by `buildGraphPromptSection` (`lib/wizard/graph-evaluator.ts`) require a 2-option `show_suggestions` picker; the educator's click triggers an explicit `update_setup({ progressionMode })` call. `setupData.curriculumPath` only determines the default ordering of the picker (`"authored"` → `["Let learners pick (recommended)", …]`; otherwise `["AI directs the sequence", …]`). `courseRefDigest.modulesAuthored` is NOT present in the runtime digest shape and must not be referenced.
...
| `progressionMode` | course | `"ai-led"` \| `"learner-picks"` | `Playbook.config.modulesAuthored` (boolean mirror) (`wizard-tool-executor.ts:796-802, 1165-1171`) | Module picker render; scheduler vs picker branching | No — wizard prompts educator to confirm before `create_course` via a 2-option `show_suggestions` picker (rule 5d, `v5-system-prompt.ts`). Default ordering keyed off `setupData.curriculumPath`. Doc-side `detect-authored-modules.ts` writes `Playbook.config.modulesAuthored` directly, but does NOT set `progressionMode` in the bag. | Wizard sets mirror; if doc declared `Modules authored: Yes` and wizard chose `ai-led`, doc-side's `modulesAuthored=true` wins because `detect-authored-modules` writes directly to `Playbook.config` and `create_course` only mirrors when wizard's `progressionMode` is set | §3 row `Playbook.config.progressionMode`; §8 L7 |
