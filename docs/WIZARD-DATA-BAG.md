...
| `strictPrerequisites` | course | boolean (default `false`) | `Playbook.config.strictPrerequisites` (#494 Slice 2.3, `json-fields.ts`) | E4 picker (`lib/curriculum/recommend-module.ts`, Slice 2.5) — hard-lock vs. soft-warning modal when a module's prerequisites are unmet | No (wizard-settable only — graph node `strictPrerequisites`) | Wizard wins (last writer) | §3 row `PlaybookConfig.strictPrerequisites` |
...
| `qualificationAnchor` (DERIVED, not user-entered) | course (auto) | string slug (e.g. `sias-cio-cto-v6`) or null | `Curriculum.qualificationAnchor` via `deriveQualificationAnchor(qualificationBody, qualificationReference)` at Curriculum-creation sites (3 routes refactored in #1081 Slice 2B.2: `apps/admin/app/api/subjects/[id]/curriculum/route.ts`, `lib/wizard/apply-projection.ts::ensureCurriculum`, variant Curriculum route) | Sibling lookup (read-only) via `findCurriculumByAnchor(anchor, domainId)` in `lib/curriculum/find-sibling-curricula.ts`. CI guard enforces slug-set + LO-ref-set agreement (`scripts/check-anchor-divergence.ts`). | No — derived in code from `qualificationBody` + `qualificationReference`. AI cannot write this field directly (rejected as unknown setup key by `validate-setup-fields.ts`). | n/a — deterministic | §3 row `Curriculum.qualificationAnchor` |
...
| `completionMode` (#494 E2 Slice 2.3) | `PlaybookConfig.completionMode` exists in `lib/types/json-fields.ts` and the AI sees it referenced in CONTENT-PIPELINE.md §3, so it assumes the wizard can set it. It cannot. **Author-only** — lives directly on `Playbook.config` via the course-design tab or the course-ref parser. The wizard validator REJECTS `update_setup({ completionMode })`. | Set in `Playbook.config.completionMode` directly (`"all-modules"` \| `"terminal-only"` \| `"any"`). Read via `lib/curriculum/course-completion.ts::readCourseFlags`. | ❌ NOT auto-corrected — validator REJECTS as unknown. Behaviour is deliberate. |
...
| 2026-05-18 | #494 E2 Slice 2.3 — added `strictPrerequisites` (wizard-settable, group `course`) to §3 master table and `completionMode` (author-only) to §4 hallucination table. Helper: `lib/curriculum/course-completion.ts::readCourseFlags`. |
