...
**Rubric-only band-descriptor pass (#564, 2026-05-19):** The #447 exclusion from *goal* projection still stands — rubric subtypes never create Goals, BehaviorTargets, or CurriculumModule rows. BUT a **separate, second projection pass** runs after the main `COURSE_REFERENCE` loop and parses rubric documents *only* for their per-band descriptor strings, writing them onto existing skill `Parameter` rows.

- **Parser:** `lib/wizard/parse-rubric-bands.ts::parseRubricBands` — scans the markdown body for `## RUB-<CODE>: <Criterion Name>` headings followed by `| Band | Descriptor |` tables. Pure, no DB / no AI.
- **Writer:** `lib/wizard/apply-projection.ts::writeBandThresholds` — updates `Parameter.config.bandThresholds` (a `{ [bandNumber: string]: string }` map) on each matched skill parameter. Idempotent. **Does NOT create new Goals, BehaviorTargets, or CurriculumModule rows.** Does NOT create new Parameter rows either — only writes onto rows the main loop already produced.
- **Orchestration:** `lib/wizard/run-projection-for-playbook.ts` — pipeline order is strict: main `COURSE_REFERENCE` loop runs FIRST (creates skill Parameters from `### SKILL-NN` blocks), rubric pass runs AFTER (writes `bandThresholds` onto those existing Parameters).
- **Match heuristic — RUB code ↔ Parameter suffix (case-insensitive), scoped to `parameterId LIKE 'skill_%'`:**
  - rubric `## RUB-FC: Fluency & Coherence` → matches `skill_fluency_and_coherence_fc`
  - rubric `## RUB-LR: Lexical Resource` → matches `skill_lexical_resource_lr`
  - rubric `## RUB-P: Pronunciation` → matches `skill_pronunciation_p`
- **Source-of-truth for the descriptor map:** `Parameter.config.bandThresholds`. The numeric tier→band mapping (Approaching Emerging ≈ Band 3, Secure ≈ Band 7+) stays on `SKILL_MEASURE_V1.contract.json::tierBands`; the per-band descriptor *text* lives on the Parameter row. See the `bandDescriptorsSource` block in that contract for the educator-facing rule (Skills Framework `(SUFFIX)` parenthetical MUST match the rubric `## RUB-<SUFFIX>:` heading).
