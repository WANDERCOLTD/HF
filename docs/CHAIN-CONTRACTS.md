...
| **I-VP2 Tool definitions are spec-driven** | The voice-call tool list MUST be loaded from the `TOOLS-001` `AnalysisSpec` at call-start and converted to the active provider's wire format by the adapter; the `VAPI_TOOL_DEFINITIONS` TypeScript constant at `app/api/vapi/tools/route.ts:913` MUST NOT exist. **Today: GAP — resolved by #1019.** Counter `vapiToolDefinitionsConstantPresent` (target 0) drives the work; #1026 already extends `scripts/generate-ai-capabilities.ts` to scrape this 4th registry so the CI drift gate covers it. |
| **I-VP3 End-of-call normalisation populates canonical fields** | The webhook adapter MUST write the eight provider-shaped fields into canonical `Call.voice*` columns (`voiceDurationSeconds`, `voiceCostUsd`, `voiceEndedReason`, `voiceAnalysisSummary`, `voiceSuccessEvaluation` — keep typed; per TL #1015 review, do NOT shove these into the JSON blob) with any provider-specific extras in `Call.voiceProviderRaw Json`. The current `Call.vapi*` columns (`schema.prisma:1457-1462`) MUST NOT exist. **Today: GAP — resolved by #1020.** Counter `vapiNamedColumnsOnCallModel` (target 0) drives the work. |
| **I-VP4 Webhook auth is per-provider, isolated** | Each provider's secret-validation scheme lives at `lib/voice/providers/<slug>/auth.ts`; no cross-provider auth fallback or shared scheme. **Today: SATISFIED at structure level** (`lib/vapi/auth.ts` is isolated; #1017 relocates it under `lib/voice/providers/vapi/auth.ts` with no behaviour change — the directory name "vapi" is correct because it IS the VAPI adapter). Asserted so a future second provider doesn't accidentally re-use VAPI's HMAC scheme. |
| **I-VP5 Tool-call callback adapter normalises to a canonical context** | The provider-shaped tool-call payload (today VAPI: `body.message.toolCallList[i].id` / `.function.name` / `.function.arguments`) MUST be normalised by the adapter to a canonical `ToolExecutionContext` before any handler dispatch in `app/api/vapi/tools/route.ts` runs. Per TL #1015 review: this contract covers BOTH inbound tool callbacks AND outbound reach-ins (`send_text_to_caller`, `share_content`, `request_artifact`) — the current code branches on `!!externalId` for outbound dispatch, which is the same anti-pattern #1025 kills inbound. **Today: GAP — resolved by #1017** (inbound) and reinforced by #1023 (SIM exercises at least one outbound reach-in through the adapter). |
