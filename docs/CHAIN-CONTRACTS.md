...
| Field | Value |
|---|---|
| **Producer** | `lib/voice/end-session.ts::endSession` (Slice 3) — fires `runSpecDrivenPipeline` asynchronously on commit; `lib/voice/reconciler.ts::reconcileCarryThrough` (Slice 5, cron every 60s) — re-fires COMPOSE on orphan Sessions with `partialFailureMode: "minimal"`. |
