...
#### Link 3 pre-condition — Call.playbookId set at pipeline-entry (#1333)

**Added 2026-06-08 (#1333). The `(callerId, playbookId)` scope key the `ComposedPrompt` row depends on is established at Call-create time, BEFORE the pipeline runs. The producer of this pre-condition is `lib/voice/create-call-entering-pipeline.ts::createCallEnteringPipeline` — the single chokepoint for pipeline-entry Call creation.**

| Field | Value |
|---|---|
| **Producer** | `lib/voice/create-call-entering-pipeline.ts::createCallEnteringPipeline({ callerId, source, voiceProvider, requestedModuleId? })` — Stage A (placeholder-create) of the voice + sim call lifecycle. Resolution cascade: `resolveActivePlaybookId(callerId)` → explicit `requestedModuleId` arg ?? `Caller.lastSelectedModuleId` ?? null → `resolveModuleByLogicalId(curriculumId, slug)` ?? `resolveDefaultModuleForCaller(callerId)`. Returns `{ playbookId: null, ... }` on missing enrollment rather than throwing — three legitimate zero-enrollment paths (sim-runner harness, onboarding-call route, brand-new caller pre-enrollment) rely on this. |
| **Consumer** | Every COMPOSE-stage reader: `app/api/calls/[callId]/pipeline/route.ts::stageExecutors.COMPOSE` and `app/api/callers/[callerId]/compose-prompt/route.ts`. `persistComposedPrompt` enforces single-active-per-`(callerId, playbookId)` — a NULL `playbookId` produces a scopeless `ComposedPrompt` that no `(callerId, playbookId)` lookup can match back to the course. |
| **Invariant** | Every Call row that enters the pipeline carries `playbookId`, `requestedModuleId`, and `curriculumModuleId` at creation time (when resolvable). When the caller has no ACTIVE enrollment, all three fields are NULL together — never one populated and the others NULL. |
| **Enforcement** | Three layers: (1) **Builder chokepoint** — every pipeline-entry write goes through `createCallEnteringPipeline`; the two production routes `POST /api/voice/calls/outbound-dial` and `POST /api/voice/calls/start` both adopt it. (2) **ESLint rule** `hf-call/no-bare-call-create` (`eslint-rules/no-bare-call-create.mjs`, error severity from day 1) blocks bare `prisma.call.create` outside an explicit 11-entry allow-list (the builder itself, its tests, the sim path that already has correct FK scoping, the harness routes, the offline-import paths, the sim-drive-call CLI, the Stage C persistEndOfCall handler, and seed scripts). Any new Call-creation site must either route through the builder OR justify a documented bypass + add to the allow-list. (3) **SQL detector** `voice-call-null-playbook-attribution` in `apps/admin/scripts/check-fk-consistency.ts` — WARN-only, reports the count of voice Calls with `playbookId = NULL` post-end; does NOT fail CI, does NOT backfill (pre-fix orphans stay as forensic evidence per the issue spec). |
| **Test** | `tests/lib/voice/create-call-entering-pipeline.test.ts` — six unit vitests covering the resolution cascade, the no-enrollment path, the explicit-arg-wins case, the `lastSelectedModuleId` fallback, the `voiceProvider=null` (sim) path, and the `outbound-dial` VAPI-error rollback. Plus `tests/integration/sessions/1333-outbound-dial.test.ts` — end-to-end seed → hit `/api/voice/calls/outbound-dial` → assert DB FKs populated. Plus `scripts/proof-1333-outbound-dial.ts` — operator-runnable proof against hf_sandbox. |
| **Memory doc** | This row; `.claude/rules/ai-to-db-guard.md` Existing Guards row for `createCallEnteringPipeline` + ESLint guard. |
| **Audit counter** | `voice-call-null-playbook-attribution` SQL check in `check-fk-consistency.ts`. Trend should plateau (pre-fix population stays) and never grow — growth = builder leak. |
| **Reinforced by** | #1333 (this row — builder + ESLint rule + WARN-only detector + integration test + proof script). Live evidence: Bertie Tallstaff (`ae3362f0-3e66-4e49-96f1-d83e10bce321`) Calls 2 + 3 on hf_sandbox 2026-06-08 — `outbound-dial` route was hand-rolling `prisma.call.create({ data: { callerId, source, voiceProvider, transcript } })` and silently dropping all three FKs; sibling `start/route.ts` set them; two implementations of the same stage-entry operation, one drifted. |
...
| **Reinforced by** | #819 (initial fan-out attempt, superseded); #825 (foundation: schema + staleness check + autoComposeForCaller + compose-prompt wire-in); #826 (Playbook.config helper pivot); #827 (8 Playbook.config writers); #828 (Domain); #829 (AnalysisSpec); #830 (BehaviorTarget + CallerTarget + Caller identity); #831 (StalePromptPill UI); #834 (curriculum / LO / assertion writers); #1429 (demo-policy eager-reprompt carve-out, this row's `Demo policy carve-out`). |
| **Demo policy carve-out — `CallerPlaybook.policyMode='demo'`** | The accepted race-window above leaves a "next-call-but-one" worst case for the learner whose touchpoint commits between an educator save and the recompose. Production tolerates that latency; demo / test loops do not — an operator places a test call seconds after tweaking a setting and expects the change to ship NOW. #1429 adds the `CallerPlaybook.policyMode` column (literal-string union, default `"production"`); `/api/intake/v2/admin-test-enrol` writes `"demo"`. Three educator-intent call sites (`lib/playbook/update-playbook-config.ts`, `lib/compose/bump-curriculum-fanout.ts` ONCE-after-the-loop, `lib/chat/admin-tool-handlers.ts::update_behavior_target`) fire `void triggerEagerRepromptForDemoCallers(playbookId)` from `lib/compose/eager-reprompt-on-bump.ts` AFTER their respective compose-input timestamp bump commits. The helper enumerates `CallerPlaybook(playbookId, policyMode='demo', status='ACTIVE')`, runs `autoComposeForCaller` per row under `p-limit(3)`, and emits a `[demo-reprompt]` structured log per attempt. Production callers (default `"production"`) take the lazy path — no change. Critically, the wire is at the three sites listed; it is NOT wired into `lib/compose/bump-timestamp.ts` itself (called from ~10 routes inside per-playbook loops — fan-out would multiply by playbook count). Course-Design Console surface: `<StalePromptPillForCourse />` mounts once in `CourseDesignConsole.tsx` header (via `ConsoleShell.headerBanner`) and reads `GET /api/courses/:courseId/staleness-aggregate` (30s in-memory cache, `?nocache=1` for post-fanout refetch). |
...
#### Link 3 sub-contract — Producer ↔ Consumer Pairing (transform output → renderer)

**Added 2026-06-17. Live incident: PR #1768 (Theme 10 profile capture) silently deleted 5 unrelated renderer consumer blocks during a bad merge resolution. The transforms kept producing `directive: "…"` fields; `renderPromptSummary.ts` stopped pushing them into the `voicePrompt` prose. Every IELTS Mock learner ran without the cue-card directive (the LLM didn't know to anchor on the Part 2 topic), the question-count directive, the verbatim closing line, the first-time orientation line, AND the score-delta narrator for ~24 hours before the regression was spotted.**

| Field | Value |
|---|---|
| **Producer** | Every transform under `apps/admin/lib/prompt/composition/transforms/**` that emits an output object containing a `directive: "…"` field (the canonical shape for an LLM-bound instruction). Today: `instructions.ts` (module_question_target, module_cue_card, module_orientation_line), `offboarding.ts` (moduleClosingLine). Loader-side equivalents (`loaders/priorCallFeedback.ts`) producing `summary` + `priorCriterionScores` for direct prose render fall under the same contract. |
| **Consumer** | `apps/admin/lib/prompt/composition/renderPromptSummary.ts::renderProviderPrompt` — the function that converts the structured `llmPrompt` JSON into the prose string VAPI hands to the LLM. EVERY directive that needs to influence the live tutor MUST have a `parts.push(...directive)` call in this renderer. The JSON shape is internal; the prose is what reaches the model. |
| **Invariant** | For every directive-bearing producer key K, the renderer MUST contain a reference to `llmPrompt.<section>?.<K>` (or the loader-output equivalent for non-section data). Three-layer enforcement: (1) **ESLint** `hf-compose/composition-directive-needs-renderer` (error severity, day 1) fires at edit time on any `transforms/*.ts` containing a `directive: "…"` field UNLESS the file carries the sentinel comment `// @renderer-consumed-at lib/prompt/composition/renderPromptSummary.ts`. (2) **Composition coverage vitest** at `tests/lib/prompt/composition/coverage-producer-consumer.test.ts` walks the static `PAIRS` manifest AND wide-sweeps every `transforms/*.ts` for orphan `directive:` fields not in the manifest. (3) **Lattice rule** `.claude/rules/lattice-survey.md` §"Producer ↔ consumer pairing — deeper layer" — author discipline + reasoning anchor. |
| **Failure mode** | Producer-only key — transform builds a `directive` string but the renderer never pushes it into `parts`. The structured `llmPrompt.instructions.<key>.directive` exists; the prose `voicePrompt` does not contain it; VAPI sends a prompt missing the operator-tuned behaviour; learner gets a degraded session. Same shape as the `categoryToTeachMethod` fallback class (#605) — silent code-side defaults masking missing behaviour — but at a deeper layer (transform vs renderer instead of transform vs setting). |
| **Test** | `tests/lib/prompt/composition/coverage-producer-consumer.test.ts` — 7 cases: 5 known directive-bearing pairs (module_question_target, module_cue_card, moduleClosingLine, module_orientation_line, priorCallFeedback) + 1 "tracked count" smoke pin + 1 wide-sweep that fails on any new orphan. ESLint rule pinned by `tests/eslint-rules/composition-directive-needs-renderer.test.ts` (11 cases — valid sentinel forms, invalid missing-sentinel forms, non-string value pass-through, file-scope allow). |
| **Memory doc** | `.claude/rules/lattice-survey.md` §"Producer ↔ consumer pairing — deeper layer: transform vs renderer"; this row; the file header on `lib/prompt/composition/transforms/instructions.ts` + `transforms/offboarding.ts` (sentinel comments). |
| **Audit counter** | None today — the structural guard (ESLint + coverage vitest) is preventative, not observational. If a corresponding live drift is ever observed in production prompts, file a counter `composeDirectiveNotPushed` (target 0) in `apps/admin/scripts/audit-epic-100.ts`. |
...
| Field | Value |
|---|---|
| **Producer** | COMPOSE-stage output (`ComposedPrompt.llmPrompt`) + the active `VoiceProvider` adapter (introduced in #1017 — today the VAPI-shaped logic at `app/api/vapi/{assistant-request,tools,webhook,knowledge}/route.ts`). |
| **Consumer** | The live voice provider's HTTP surface (today VAPI's `assistant-request` / `serverUrl` / `customer.number` / `end-of-call-report` events; tomorrow whatever RetellAI / Telnyx / Daily / Realtime expect). |
| **I-VP1 Composed prompt is provider-agnostic** | `ComposedPrompt.llmPrompt` MUST contain no VAPI-specific tokens; the same byte stream serves any provider's system-message slot. **Today: SATISFIED** (proof: `scripts/sim-drive-call.ts` produces a multi-turn session from the same prompt with no VAPI involvement; `lib/channels/capabilities.ts::resolveContentChannelFromSource` already maps `"vapi"` → `"voice"`). Asserted as an invariant so no future transform regresses by injecting `"VAPI"` into prompt text. |
| **I-VP2 Tool definitions are spec-driven** | The voice-call tool list MUST be loaded from the `TOOLS-001` `AnalysisSpec` at call-start and converted to the active provider's wire format by the adapter; the `VAPI_TOOL_DEFINITIONS` TypeScript constant MUST NOT exist. **Today: SATISFIED** — #1019 migrated the constant into `docs-archive/bdd-specs/TOOLS-001-voice-tool-definitions.spec.json`; `lib/voice/load-tool-definitions.ts` reads it at call-start with a safe `[]` fallback. Build-time enforced by ESLint rule `hf-voice/no-vapi-tool-definitions-const` (#1024). Counter `vapiToolDefinitionsConstantPresent` reads 0. |
| **I-VP3 End-of-call normalisation populates canonical fields** | The webhook adapter MUST write the provider-shaped fields into canonical `Call.voice*` columns (`voiceDurationSeconds`, `voiceCostUsd`, `voiceEndedReason`, `voiceAnalysisSummary`, `voiceStructuredData`, `voiceSuccessEvaluation` — typed; per TL #1015 review, do NOT shove these into the JSON blob) with any provider-specific extras in `Call.voiceProviderRaw Json`. The pre-rename `Call.vapi*` columns MUST NOT exist. **Today: SATISFIED** — #1020 renamed all 6 columns + added `voiceProviderRaw Json` for forensic extras. Build-time enforced by ESLint rule `hf-voice/no-vapi-column-ref` (#1024). Counter `vapiNamedColumnsOnCallModel` reads 0. |
| **I-VP4 Webhook auth is per-provider, isolated** | Each provider's secret-validation scheme lives at `lib/voice/providers/<slug>/auth.ts`; no cross-provider auth fallback or shared scheme. **Today: SATISFIED** — `lib/voice/providers/vapi/auth.ts` (moved from `lib/vapi/auth.ts` in #1017) is the sole entry point for VAPI HMAC verification; routes invoke it via `VoiceProvider.verifyInboundRequest`. The directory name "vapi" is correct because it IS the VAPI adapter. |
| **I-VP5 Tool-call callback adapter normalises to a canonical context** | The provider-shaped tool-call payload (VAPI: `body.message.toolCallList[i].id` / `.function.name` / `.function.arguments` — also tolerated: legacy `.functionCall.{name, parameters}`) MUST be normalised by the adapter to a canonical `NormalisedToolCallBatch` (`{ toolCalls: [{ toolCallId, funcName, args }], customerPhone }`) before any handler dispatch in `app/api/vapi/tools/route.ts` runs. **Today: SATISFIED** — `app/api/vapi/tools/route.ts` calls `provider.normaliseToolCallList(body)` (#1017) for inbound; outbound reach-ins (`send_text_to_caller`, `share_content`, `request_artifact`) now flow through `routeToolCall` in `lib/voice/tool-router.ts` (#1023). SIM exercises at least one outbound reach-in via the canonical path. |
| **Enforcement** | (1) Static counters in `apps/admin/scripts/audit-epic-100.ts` — `vapiNamedColumnsOnCallModel` (I-VP3) and `vapiToolDefinitionsConstantPresent` (I-VP2). Both `kind: "invariant"`, target 0. Filesystem-only so they work in CI without DB. (2) CI step 6 (epic-100 audit baseline check) gates merges; baseline JSON updated in the same commit as the counter definitions, per the same convention #1008 used. (3) Arch-checker rule `hf-voice/no-raw-vapi-source-branch` (lands in #1024) blocks new `Call.source === "VAPI"` branches outside `lib/channels/capabilities.ts::resolveContentChannelFromSource`. (4) `docs/AI-CAPABILITIES.md` drift gate (#1026 extension) covers `VAPI_TOOL_DEFINITIONS` so any change to the constant before #1019 ships still routes through CI. |
| **Pipeline COMPOSE compatibility** | Untouched. This sub-contract sits BELOW Link 3's COMPOSE → LLM output invariants — the LLM (and therefore the voice provider) consumes `ComposedPrompt.llmPrompt` unchanged. The TUNER → COMPOSE staleness carve-out, the COMPOSE → LLM invariant runner (`runComposeInvariants`), and the per-section transform contracts are all unaffected. Adding a second provider does not add a new pipeline stage; it adds a new adapter consuming an unchanged pipeline output. |
| **Test** | (1) `tests/lib/call-voice-provider.test.ts` (lands in #1025) — `VoiceProviderSlug` enum exists with `VAPI` value; backfill default holds. (2) Per-invariant pin tests added as the sibling stories land: I-VP1 → `tests/lib/voice/prompt-portability.test.ts` (#1017); I-VP2 → `tests/lib/voice/tools-from-spec.test.ts` (#1019); I-VP3 → migration verification test (#1020); I-VP4 → `tests/lib/voice/providers/auth-isolation.test.ts` (#1017); I-VP5 → `tests/lib/voice/tool-context-normalization.test.ts` (#1017) with outbound-reach-in coverage (#1023). |
| **Memory doc** | This row; `docs/PIPELINE.md` §1 (stages — voice provider sits below COMPOSE consumer); `docs/PIPELINE.md` §3 (non-blocking `stageErrors` semantics preserved); per-adapter file headers under `lib/voice/providers/<slug>/` once #1017 lands. |
| **Audit counter** | `vapiNamedColumnsOnCallModel` (I-VP3, target 0, current 6) and `vapiToolDefinitionsConstantPresent` (I-VP2, target 0, current 1) — both in `apps/admin/scripts/audit-epic-100.ts` + `tests/fixtures/epic-100-audit-baseline.json`. Baseline JSON updated in the same commit as the counter definitions (precedent: #1008). |
| **Reinforced by** | #1015 (epic — AnyVoice voice-provider abstraction); #1025 (foundational — `Call.voiceProvider` enum, kills `isVapiCall` coupling); #1026 (extends `generate-ai-capabilities.ts` so the CI drift gate covers `VAPI_TOOL_DEFINITIONS` until #1019 migrates it); #1017 (`VoiceProvider` adapter interface, extracts `VapiProvider`); #1019 (TOOLS-001 spec migration — drives `vapiToolDefinitionsConstantPresent` to 0); #1020 (schema rename — drives `vapiNamedColumnsOnCallModel` to 0); #1021 (webhook adapter + canonical normalisation); #1022 (knowledge adapter parity); #1023 (SIM exercises outbound reach-in through adapter); #1024 (arch-checker rules including `hf-voice/no-raw-vapi-source-branch`); #1027 (per-caller voice-provider routing — extends adapter context with `callerId`); #1028 (voice cost telemetry — new `VOICE` `UsageCategory`); #1029 (VAPI comment + SIM-prompt cleanup). |

### Link 3b — SESSION → COMPOSE (carry-through + terminal fallback)

**Added 2026-06-08 (#1341 / epic #1338 Slice 0). Formalises the contract between every learner Session (voice call, sim run, text chat, enrolment, assessment) and the ComposedPrompt that the next Session of the same caller starts from. Slice 0 ships the schema only; the invariants are enforced by Slices 3 + 5. Source: Bertie Tallstaff's hf_sandbox session (2026-06-08) surfaced a ghost Call row with no `callSequence` or `endedAt`, a Tune-tab header showing "call #4" while `Call.callSequence = 3`, and in-memory intake-chat Q&A lost on dev-server restart (`lib/intake/session-store.ts:1-18` Map). Same anti-pattern class as #1008 (silent code-side defaults masking missing data) and #1006 (compose-time fabrication when state is incoherent).**

| Field | Value |
|---|---|
| **Producer** | `lib/voice/end-session.ts::endSession` (Slice 3) — fires `runSpecDrivenPipeline` asynchronously on commit; `lib/voice/reconciler.ts::reconcileCarryThrough` (Slice 5, cron every 60s) — re-fires COMPOSE on orphan Sessions via `lib/voice/carry-through-compose.ts::carryThroughCompose` (the shared minimal-mode COMPOSE-fallback helper, also invokable from the live pipeline by POSTing `partialFailureMode: "minimal"` to `/api/calls/:callId/pipeline`). The fallback path reads ONLY the I-CT2 cascade (`lib/voice/resolve-used-prompt.ts`) — never the LLM — and stamps `ComposedPrompt.inputs.partialFailureMode = "minimal"` so the Tune tab surfaces the "↻ reconciled" badge. |
| **Consumer** | `createSession` (Slice 3) — reads the per-caller ACTIVE ComposedPrompt cascade at session-start; written into `Session.usedPromptId`. |
| **Data shape** | `Session` row (parent of voice call / sim / text chat / enrolment / assessment) + matching `ComposedPrompt(triggerSessionId = Session.id)` row. Per-(callerId, kind) sequence enforced by `@@unique([callerId, kind, sequenceNumber])`; atomic counter in `CallerSequenceCounter`. `Call.sessionId` is the 1:1 join key; `Call.callSequence` (legacy) is preserved during the transition window. |
| **I-CT1 Carry-through (eventually consistent, 60s budget)** | Every `Session` row with `endedAt IS NOT NULL` MUST have a corresponding `ComposedPrompt(triggerSessionId = Session.id, status='active')` within 60 seconds of `endedAt`. Enforced by: (1) `lib/voice/end-session.ts::endSession` fires `runSpecDrivenPipeline` asynchronously on commit (Slice 3); (2) `lib/voice/reconciler.ts::reconcileCarryThrough` (Slice 5, cron every 60s) scans for orphans where `Session.endedAt < NOW() - 60s` AND no matching `ComposedPrompt` exists, then re-fires COMPOSE with `partialFailureMode: "minimal"`; (3) `scripts/check-fk-consistency.ts::session-without-composed-prompt` WARN-only invariant check (Slice 1). 60s budget mirrors `lib/voice/poll-stale-calls.ts:73`'s `staleAfterMs` so the two reconcilers' cadences don't drift. |
| **CarryThrough soft-acknowledge rule** | When the prior Session has `status='FAILED'` or `status='GHOST'`, the new ComposedPrompt MUST include the failure context in its template so the next interaction soft-acknowledges ("Last time we got cut off — let's pick up from..."). Same anti-fabrication principle as I-C3 (no memory-less reminisce) on Link 3 — the prompt should not invent narrative continuity when none exists. Enforced by a new transform `lib/prompt/composition/transforms/priorSessionFailureContext.ts` (Slice 3) gated on the prior `Session.status`. |
| **I-CT2 Terminal fallback (no caller starts prompt-less)** | `createSession` (Slice 3) MUST resolve `usedPromptId` via this unconditional cascade — the third step is the terminal guarantee: (1) `ComposedPrompt(triggerSessionId = previous Session.id, status='active')` — fast path when I-CT1 has converged; (2) most recent `ComposedPrompt(callerId = caller.id, status='active')` regardless of `triggerSessionId` — covers the I-CT1 race window where the carry-through hasn't fired yet; (3) bootstrap from the `Session(kind='ENROLLMENT').producedComposedPromptId` — written by `lib/voice/stamp-enrollment-session-prompt.ts::stampEnrollmentSessionPrompt` after `autoComposeForCaller` succeeds at enrolment time (#1420). Pre-#1420 this step was structurally dead: the post-tx auto-compose hook was suppressed by the `!tx` guard in `lib/enrollment/index.ts::enrollCaller` for the `/api/join/[token]` new-user path and `/api/invite/accept` (both call `enrollCaller(..., tx)` inside `$transaction`). Brand-new callers' Call 1 fell through to the `build-assistant-config.ts` hardcoded fallback. #1420 closes the gap: (a) both routes fire `autoComposeForCaller` POST-tx fire-and-forget, (b) `stampEnrollmentSessionPrompt` retroactively links the resulting prompt to the ENROLLMENT Session row, (c) `lib/voice/reconcile-missing-bootstrap.ts::reconcileMissingBootstrap` runs every 60s from the same cron as `reconcileCarryThrough` to backstop the process-crash case. Step (3) is now the durable terminal anchor: a caller with prior history can never start a Session with `usedPromptId IS NULL`. Mirror of #1008's I-C1 module-lock invariant. |
| **Enforcement** | Schema-level: `Session.usedPromptId` is nullable in Slice 0 only because the migration backfill cannot perfectly reconstruct the prompt used by every legacy Call row; Slice 4 considers tightening to NOT NULL once the I-CT2 cascade has run on every active caller. Build-time: arch-checker rule `hf-session/no-prompt-less-create` (Slice 3) blocks `prisma.session.create({ ... usedPromptId: undefined })` in any path outside `createSession`. Run-time: `createSession` throws if step (3) of the cascade returns null. Reconciler (Slice 5) emits structured logs + audit counter `sessionWithoutComposedPrompt` (target 0 after 60s budget). |
| **Schema-only enforcement check (Slice 0)** | Until Slice 3 wires the builders, no application code reads or writes `Session`. Verified by `grep -rE 'prisma\.session\.' apps/admin/app apps/admin/lib apps/admin/components --include="*.ts" --include="*.tsx" \| grep -v tests/` returning zero hits. Existing NextAuth references (the wrapper in `lib/auth/with-renamed-session-model.ts` and one delete in `app/api/admin/users/route.ts`) read `prisma.authSession.*` instead. |
| **Test** | Schema-only fixture pair at `tests/fixtures/sessions/1341-schema-pre.json` (existing Call rows shape) + `1341-schema-post.json` (expected `Session` + `Call.sessionId` + `CallerSequenceCounter` post-migration shape); integration test at `tests/integration/sessions/1341-schema.test.ts` programmatically applies the migration to an ephemeral DB and asserts shape equivalence; operator-runnable proof at `scripts/proof-1341-schema.ts` (run against hf_sandbox post-deploy). Per-invariant pin tests land with the builders: I-CT1 → `tests/lib/voice/reconciler-carry-through.test.ts` (Slice 5); I-CT2 → `tests/lib/voice/create-session-prompt-cascade.test.ts` (Slice 3). |
| **Memory doc** | This row; `memory/flow-call-lifecycle.md` (Session is the new canonical parent of Call); per-builder file headers under `lib/voice/end-session.ts` and `lib/voice/reconciler.ts` once Slice 3 + 5 land. |
| **Audit counter** | `sessionWithoutComposedPrompt` (Slice 5, I-CT1, target 0 after 60s budget — measured at 5min cadence by the audit job). `callerSequenceCounterMissing` (Slice 3, target 0). `sessionPromptLessCreate` (Slice 3, I-CT2 violations caught by `createSession`'s pre-persist throw — target 0, alarm-on-non-zero). All `kind: "invariant"`. |
| **Reinforced by** | #1338 (epic — Session parent + counter + reconciler); #1341 (this PR — Slice 0 schema, builders absent); #1342 (Slice 3 — `createSession` / `endSession` builders, full route cutover); planned: Slice 1 (`FailureLog` model + `check-fk-consistency.ts` warn rule); Slice 2 (PrismaEventStore replaces `IntakeSession` Map at `lib/intake/session-store.ts:1-18`); Slice 4 (`Session.learnerFacingNumber` as single source of truth — drops `Call.callSequence`); Slice 5 (reconciler worker + I-CT1 vitest); **#1420 (follow-on — closes the tx-suppression gap in `enrollCaller`; adds `stampEnrollmentSessionPrompt`, `reconcileMissingBootstrap`, `scripts/backfill-enrollment-bootstrap.ts`, `[CRITICAL]` log in `build-assistant-config.ts`)**. |
| **Slice 0 risk callouts** | (a) Backfill join key (`startedAt = createdAt` + `sequenceNumber` tie-break) cannot perfectly disambiguate Calls that share both `(callerId, createdAt)` AND have NULL `callSequence`. Migration includes a last-resort `ROW_NUMBER` fallback (step 6b) — verify zero remaining NULLs via `proof-1341-schema.ts`. (b) `Session.playbookId` (nullable) is a sequencing / config-snapshot field, not a content-scoping join — does NOT open a new cross-course content leak (ENTITIES.md §9 Leak A/B/C/E7/E8 unchanged). Flagged for Tech Lead review on PR. (c) NextAuth `model Session` was renamed to `AuthSession` so the canonical learner table can take the simple name; PrismaAdapter's hardcoded `p.session.*` is redirected via `lib/auth/with-renamed-session-model.ts`. JWT strategy means these methods are dormant in practice. |
