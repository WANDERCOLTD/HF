...
| [`lattice-chain-closure`](#guard-lattice-chain-closure) | **6th Coverage-pillar gate.** End-to-end chain-closure assertion. Walks each chain declared in `docs/lattice-chains.json` link-by-link and asserts adjacent-link KEY consistency: every producer's `outputKey` matches the next consumer's `consumesKey` (modulo `{placeholder}` substitution). Catches the failure mode per-link Coverage gates can't see — a chain that passes every per-link gate yet dies semantically because adjacent links use mismatched key shapes (the worked example: AGGREGATE writes `behavior_profile:engagement:abstract_concrete`, ADAPT reads `behavior_profile:engagement:abstractness` — both per-link gates pass; chain dies). 4 seed chains at land time (`beh-aggregate-cascade`, `parameter-loop`, `compose-producer-consumer`, `journey-setting-coverage`); ratchet `EXPECTED_TOLERATED_DRIFT_TOTAL = 0`. Paired with `lattice-self-maintenance.test.ts` for .md ↔ .json parity. | #2057 | **a** |
...
| <a id="guard-lattice-chain-closure"></a>`tests/lib/lattice-chain-closure.test.ts` (rule [`.claude/rules/lattice-chain-closure.md`](../../.claude/rules/lattice-chain-closure.md)) | **6th Coverage-pillar member.** End-to-end Lattice chain-closure gate. Walks each chain declared in [`docs/lattice-chains.json`](../lattice-chains.json) link-by-link and asserts adjacent-link KEY consistency: every producer's `outputKey` must literally match the next consumer's `consumesKey` (modulo `{placeholder}` substitution). Catches the failure mode per-link Coverage gates can't see — a chain that passes every per-link gate yet dies semantically because two adjacent links use mismatched key shapes. **Worked example (from #2057's body):** MEASURE writes `CallScore.parameterId = "BEH-ABSTRACT-CONCRETE"`, AGGREGATE reads `sourceParameter = "BEH-ABSTRACT-CONCRETE"` ✓; AGGREGATE writes `behavior_profile:engagement:abstract_concrete`, ADAPT reads `behavior_profile:engagement:abstractness` ✗ — every per-link gate passes; chain dies between AGGREGATE→ADAPT. Manifest captures 4 seed chains at land time (`beh-aggregate-cascade`, `parameter-loop`, `compose-producer-consumer`, `journey-setting-coverage`). 11 vitests (schema-load, kebab-case-id, ≥2-links, file-existence, adjacent-key-consistency, drift-ratchet, non-empty-reason, valid-indices, no-stale-drift, terminal-reaches-set, distribution-sanity). Ratchet `EXPECTED_TOLERATED_DRIFT_TOTAL = 0`. Paired with `lattice-self-maintenance.test.ts` for .md ↔ .json parity. Parent epic: #1909 (extension). | **a** |
