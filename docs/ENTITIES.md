...
| `ContentAssertion` | `schema.prisma::model ContentAssertion` (line 3704) | Atomic teaching point | AssertionMedia | `subjectSourceId: String?` (line 3716) — **see §6 invariant**; `teachMethod: String?` (line 3910) — TS union `TeachMethod` in `lib/content-trust/resolve-config.ts` (9 values, incl. `tutor_instruction` since #605 — sentinel for INSTRUCTION_CATEGORIES, never rendered to learner). |
...
| `Curriculum` | `schema.prisma::model Curriculum` (line 2165) | Owned by Playbook (since #181) | CurriculumModules | `playbookId`; `qualificationAnchor: String?` (#1081 Slice 2B.1) — labelling field; groups Curricula teaching the same regulated qualification. **NOT a sharing mechanism** (use `PlaybookCurriculum(role: linked)` for that — see §4.1 PlaybookSource boundary). Indexed. Derived deterministically from `qualificationBody` + `qualificationReference` via `lib/curriculum/qualification-anchor.ts::deriveQualificationAnchor`. CI guard `scripts/check-anchor-divergence.ts` enforces module-slug-set + LO-ref-set agreement between siblings sharing an anchor. |
...
| I8 | **#605 — tutor-instruction TeachMethod separation.** `ContentAssertion.teachMethod` MUST equal `"tutor_instruction"` whenever `category ∈ INSTRUCTION_CATEGORIES` (`lib/content-trust/resolve-config.ts` lines 62–77). Inverse: `"tutor_instruction"` is the only allowed value for those categories. Tutor directives must never render as learner activity prompts (root cause: pre-#605 the fallback in `categoryToTeachMethod` silently routed them to `"recall_quiz"`). | `categoryToTeachMethod()` short-circuit guard at the assignment boundary; `assertNoLearnerMethodOnInstructionCategory()` invariant in `base-extractor.ts` + `extract-assertions.ts`. Audit counter `recallQuizOnInstructionCategories` (target 0) in `scripts/audit-epic-100.ts`. |
| I9 | **#607 — exactly one primary Subject per Playbook.** `wizard-tool-executor.create_course` (and any future course-creation path) MUST end with exactly one `PlaybookSubject` row per `playbookId`, pointing at the course-scoped Subject (`slug = {domain.slug}-{slug(playbook.name)}-{disciplineSlug}`). Composition paths that touch subjects (`quick-launch/analyze`, etc.) MAY pre-link domain-level subjects; the wizard displaces them. DB does NOT enforce — `@@unique([playbookId, subjectId])` only blocks same-subject duplicates, not cross-subject coexistence. | Application: `unlinkNonPrimaryPlaybookSubjects(playbookId, keepSubjectId)` in `lib/knowledge/cleanup-placeholder-subjects.ts`, called from `wizard-tool-executor.ts::create_course` step 4b after the course-scoped Subject is linked. Audit counter `duplicatePlaybookSubjects` (target 0) in `scripts/audit-epic-100.ts` — detects playbooks where a course-scoped and a non-course-scoped Subject coexist. |
| I10 | **#836 — `@map()` Prisma aliases are FK tripwires.** When a Prisma field name (`callerIdentityId`) differs from the physical DB column name (`@map("callerId")`), readers can mistakenly assume the referenced ID type matches the original entity name. Concrete instance: `BehaviorTarget.callerIdentityId @map("callerId")` (`schema.prisma:549`) references `CallerIdentity.id`, NOT `Caller.id`. Pre-fix, `lib/tolerance/resolve-tolerance.ts` passed a `Caller.id` into that column and layer 1 of the mastery cascade silently never fired in prod. **Rule:** every `prisma.behaviorTarget.{find*,create,update}` that filters or writes `callerIdentityId` MUST pass a `CallerIdentity.id` (canonical path: fan out via `prisma.caller.findUnique({ select: { callerIdentities } })`, or call the dedicated helper `lib/agent-tuner/write-target.ts::resolveCallerIdentityIds`). New `@map()` aliases that obscure the referenced entity should be avoided. | Application: `lib/tolerance/resolve-tolerance.ts::readCallerBehaviorTargetValue` (canonical reader with embedded fanout + MAX-tie-break across identities); `lib/agent-tuner/write-target.ts::writeCallerBehaviorTarget` (canonical writer). Cross-reference: `docs/CHAIN-CONTRACTS.md` Link 3 sub-contract FK-invariant row. Discovered via empirical repro on hf_sandbox (2026-05-25). |
...
| 2026-05-23 | **§3 + §6 — `tutor_instruction` TeachMethod sentinel (#605).** Added `teachMethod` column note to `ContentAssertion` row in §3. New invariant I8: INSTRUCTION_CATEGORIES rows MUST carry `teachMethod="tutor_instruction"` so tutor directives never render as learner activity. `categoryToTeachMethod()` short-circuits on the discriminant; `assertNoLearnerMethodOnInstructionCategory()` is run at every extraction boundary. Backfill script in `scripts/backfill-teach-methods.ts` pass 2 drains historical `recall_quiz` mis-tags. Audit counter `recallQuizOnInstructionCategories` (target 0) in `scripts/audit-epic-100.ts`. |
| 2026-05-23 | **§6 — single-primary-Subject invariant (#607).** New invariant I9: a Playbook MUST end up with exactly one `PlaybookSubject` row pointing at the course-scoped Subject. Two creation paths (`quick-launch/analyze` + `wizard-tool-executor.create_course`) used to both link subjects with no shared knowledge; the DB unique constraint only blocked same-subject duplicates, allowing different-subject coexistence (#600 RC4 IELTS Prep Lab). New `unlinkNonPrimaryPlaybookSubjects()` helper in `lib/knowledge/cleanup-placeholder-subjects.ts` enforces the invariant after the course-scoped Subject is linked. Audit counter `duplicatePlaybookSubjects` (target 0) reframed in `scripts/audit-epic-100.ts` to measure cross-subject coexistence rather than the DB-impossible same-subject pair-duplicate. |
| 2026-05-26 | **§6 — `@map()` Prisma FK tripwire invariant (#836).** New invariant I10: `BehaviorTarget.callerIdentityId @map("callerId")` aliases the physical column name and tricks readers into joining on `Caller.id` when the FK actually references `CallerIdentity.id`. Pre-fix, the mastery threshold cascade's layer 1 in `lib/tolerance/resolve-tolerance.ts` was silently dead in production. Fix: split the resolver's BehaviorTarget reader into per-scope helpers; CALLER branch fans out via `caller.callerIdentities[]` and picks `MAX(targetValue)` across identities. Cross-referenced in `docs/CHAIN-CONTRACTS.md` Link 3 sub-contract. |
