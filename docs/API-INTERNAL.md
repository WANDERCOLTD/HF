...
Selective or full runtime data deletion. Optionally re-seeds metering data.

**Auth**: session (ADMIN+) · **Scope**: `admin:write`

**Response** `200`
```json
{ ok: true, result: DemoResetResult }
```
...
Delete all ContentSources AND curriculum artifacts (curricula,

**Auth**: session (SUPERADMIN) · **Scope**: `admin:write`

**Response** `200`
```json
{ ok: true, deleted: { sources, subjects_unlinked, curricula, modules, learningObjectives, moduleProgress }, domainName }
```
...
Scoped demo reset for Abacus Academy. Removes demo-created courses,
...
**Auth**: session (ADMIN+) · **Scope**: `admin:read`

**Response** `200`
```json
{ ok: true, entities: EntityCount[], totals: { runtime: number, config: number } }
```
...
**Response** `400`
```json
{ ok: false, error } - Caller has no active enrollment, or multiple enrollments with no default and no explicit playbookId
```
...
### `POST` /api/callers/:callerId/enrollments
...
### `DELETE` /api/callers/:callerId/enrollments/:enrollmentId

Remove an enrollment entirely (hard delete).
...
|-----------|-----|------|----------|-------------|
| callerId | path | string | Yes | The caller ID |
| enrollmentId | path | string | Yes | The enrollment ID |
...
### `PATCH` /api/callers/:callerId/enrollments/:enrollmentId

Update an enrollment status (pause, resume, complete, drop).
...
| callerId | path | string | Yes | The caller ID |
| enrollmentId | path | string | Yes | The enrollment ID |
| status | body | string | No | New status: ACTIVE, PAUSED, COMPLETED, DROPPED |
...
**Response** `200`
```json
{ ok: true, enrollments: EnrollmentJourney[] }
```
...
Get all media shared with a caller across their calls. Supports sort, order, type filter, and call filter.

**Auth**: session (VIEWER+) · **Scope**: `callers:media:history`

**Response** `200`
```json
{ ok: true, media: SharedMediaItem[], total: number }
```
...
### `GET` /api/callers/[callerId]/cascade/voice

Returns the full voice-config cascade explanation for one
...
**Auth**: session | x-internal-secret · **Scope**: `pipeline:execute`
...
### `GET` /api/cohorts/:cohortId/playbooks
...
### `POST` /api/cohorts/:cohortId/playbooks
...
### `DELETE` /api/cohorts/:cohortId/playbooks/:playbookId
...
### `POST` /api/cohorts/:cohortId/playbooks/sync
...
Returns text fragments extracted from AnalysisSpec configs with category, prompt-consumed flag, and search/filter support.

**Auth**: Session · **Scope**: `content:read`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| category | query | string | No | Filter by category (identity, voice, content, etc.) |
| specSlug | query | string | No | Filter by spec slug |
| promptOnly | query | string | No | "true" to show only prompt-consumed fragments |
| search | query | string | No | Full-text search across fragment values and labels |

**Response** `200`
```json
{ fragments: [...], stats: {...}, filters: {...} }
```
...
Create a new content source with provenance metadata.

**Auth**: Session · **Scope**: `content:create`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| slug | body | string | No | Unique slug (required) |
| name | body | string | No | Display name (required) |
| description | body | string | No | Source description |
| trustLevel | body | string | No | Trust level (default: UNVERIFIED) |
| publisherOrg | body | string | No | Publisher organization |
...
### `GET` /api/content-review/counts

Returns lightweight counts for the content review queue.

**Auth**: OPERATOR · **Scope**: `content-review:read`
...
### `GET` /api/content-review/items

Returns detailed items for the content review page:

**Auth**: OPERATOR · **Scope**: `content-review:read`

**Response** `200`
```json
{ ok: true, dirtySpecs: [...], errorTasks: [...] }
```
...
### `POST` /api/content-sources/suggest

AI-fills content source metadata from a free-text description, ISBN, or URL.

**Auth**: Session · **Scope**: `content-sources:write`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| description | body | string | No | Free-text description of the content source (e.g. "CII R04 Insurance Syllabus 2025/26" or an ISBN) |

**Response** `200`
```json
{ ok, fields: { slug, name, trustLevel, publisherOrg, ... }, interpretation: string }
```
...
### `DELETE` /api/content-sources/:sourceId/assertions/:assertionId
...
### `PATCH` /api/content-sources/:sourceId/assertions/:assertionId
...
### `POST` /api/content-sources/:sourceId/assertions/bulk-review
...
### `POST` /api/content-sources/:sourceId/extract
...
Analyze multiple uploaded files as a course pack. Uses AI to group

**Auth**: OPERATOR · **Scope**: `content:write`

**Response** `200`
```json
{ ok, manifest: PackManifest }
```

---

### `POST` /api/course-pack/ingest

Ingest a confirmed course pack manifest. Creates Subjects, ContentSources,

**Auth**: OPERATOR · **Scope**: `content:write`

**Response** `200`
```json
text/event-stream — SSE progress events, final "complete" event includes
```
...
### `GET` /api/courses/:courseId/cohort-learning

Returns cohort-level learning aggregate for a course — before (survey) vs after (session scores), competency band distribution, parameter averages, checkpoint pass rates.

**Auth**: Session · **Scope**: `courses:read`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| courseId | path | string | Yes | The playbook (course) ID |

**Response** `200`
```json
{ ok: true, data: CohortAggregateData }
```

**Response** `200`
```json
{ ok: true, data: null } — No learning data (knowledge profile or no enrolled learners)
```
...
**Response** `200`
```json
{ ok, teachingMode, methods, total, contentCount, instructionCount, unassignedContentCount, reviewedCount, categoryCounts } (summary mode)
```
...
### `GET` /api/courses/:courseId/content-sources

Returns a flat list of content sources linked to this course via PlaybookSource.
...
### `GET` /api/courses/:courseId/distribution-advisory

Runs advisory checks against the course's current lesson plan.
...
Compose the prompt that would fire if a learner started a call

**Auth**: OPERATOR · **Scope**: `courses:read`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| courseId | path | string | Yes | Playbook UUID |
| callSequence | body | number | No | Override call count (1 = force first call). Default: 1. |
| requestedModuleId | body | string | No | Optional authored module to lock the session to. |
| simCallerId | body | string | No | Optional explicit caller ID to compose for. Default: pick existing learner or first caller in the course's domain. |

**Response** `200`
```json
{ ok: true, dryRun: true, promptSummary, llmPrompt, trace, metadata }
```

**Response** `400`
```json
{ ok: false, error } - No usable caller available
```
...
### `GET` /api/courses/:courseId/session-count-recommendation

Computes a session count recommendation based on the course's
...
### `POST` /api/curricula/:curriculumId/lesson-plan
...
### `PUT` /api/curricula/:curriculumId/lesson-plan
...
| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| courseId | query | string | No | Optional: playbook ID for robust source resolution via getSubjectsForPlaybook |
...
### `GET` /api/dashboard/proof-summary

Aggregate proof points for the dashboard — total students, mastery, calls,
...
### `GET` /api/domains/:domainId/extraction-config
...
### `PUT` /api/domains/:domainId/extraction-config
...
### `DELETE` /api/educator/classrooms/[id]

Soft-delete a classroom by setting isActive to false. Requires educator ownership of the cohort.
...
### `DELETE` /api/playbook-groups/:id/subjects

Remove a subject from a department/group.

**Auth**: Bearer token · **Scope**: `groups:write`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| subjectId | body | string | No | Subject ID to unlink |
...
### `GET` /api/playbook-groups/:id/subjects

List subjects linked to a department/group, with course counts.
...
### `POST` /api/playbook-groups/:id/subjects

Link a subject to a department/group.

**Auth**: Bearer token · **Scope**: `groups:write`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| subjectId | body | string | No | Subject ID to link |
...
**Response** `409`
```json
{ ok: false, error: "Subject already linked" }
```
...
Resend an existing invite email. Regenerates token and extends expiry for expired invites. Admin-only.

**Auth**: Session · **Scope**: `invites:write`

**Response** `200`
```json
{ ok: true, invite: {...}, inviteUrl: string, emailSent: boolean }
```
...
Clears all log entries from the AppLog table.
...
Toggle logging on/off and/or set which log types are enabled. Both fields are optional and can be set independently. Config is DB-backed via SystemSetting.
...
Replace the file content of an existing media asset. Validates MIME type and file size,

**Auth**: session (OPERATOR+) · **Scope**: `media:replace`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| file | body | File | No | The replacement file (multipart/form-data) |

**Response** `200`
```json
{ ok: true, media: { id, fileName, mimeType, fileSize, url }, previousFileName: string }
```
...
List all registered messaging providers. Sorted: SYSTEM

**Auth**: session ADMIN · **Scope**: `messaging-providers:read`

**Response** `200`
```json
{ ok: true, providers: MessagingProvider[] }
```

---

### `POST` /api/messaging-providers

Create a new messaging provider. `adapterKey` must match an

**Auth**: session ADMIN · **Scope**: `messaging-providers:write`
...
**Response** `409`
```json
{ ok: false, error: string } — slug or (institutionId, adapterKey) collision
```
...
### `GET` /api/callers/[callerId]/memories
...
### `GET` /api/courses/:courseId/course-reference

Returns the most recent COURSE_REFERENCE markdown document
...
**Response** `200`
```json
{ ok, reference: { id, name, markdown, createdAt } | null }
```
...
**Response** `200`
```json
{ ok: true, sections: Array<{ sectionKey: string, sectionHash: string, staleSince: string, affectedCallerCount: number }>, capped: boolean }
```
...
**Response** `200`
```json
{ ok, modulesAuthored, modules, validationWarnings, detectedFrom, hasErrors, persisted }
```

**Response** `400`
```json
{ ok: false, error: "Invalid body", issues: ZodIssue[] }
```
...
### `POST` /api/curricula/:curriculumId/reclassify-los
...
### `GET` /api/institution/context

Returns the current user's institution and primary domain IDs.
...
### `DELETE` /api/messaging-providers/[id]

Soft-disable a provider (sets enabled=false). Hard delete
...
### `GET` /api/messaging-providers/[id]
...
### `PATCH` /api/messaging-providers/[id]

Update a messaging provider. `slug` is immutable. Setting
...
**Response** `200`
```json
{ ok: true, courseName, materials: SessionMaterial[] }
```
...
### `GET` /api/system/db-target

Returns the live DB target name parsed from the runtime DATABASE_URL.

**Auth**: None

**Response** `200`
```json
{ ok: true, dbTarget: "sandbox" | "staging" | "pilot" | "prod" | null, dbName: string | null }
```
...
### `GET` /api/playbooks/:playbookId/enrollments
...
### `POST` /api/playbooks/:playbookId/enrollments/bulk

Bulk enroll callers in a playbook.
...
**Response** `200`
```json
{ ok: true, enrolled: number, errors: string[] }
```
...
### `POST` /api/playbooks/:playbookId/recompose-all

Fan-out prompt recomposition across every ACTIVE caller enrolled in a

**Auth**: Session · **Scope**: `playbooks:compose`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| playbookId | path | string | Yes | Playbook UUID |
| triggerType | body | string | No | Default "TUNER_FANOUT" |

**Response** `200`
```json
{ ok: true, total, succeeded, failed, errors: [...] }
```
...
### `POST` /api/playbooks/:playbookId/targets/suggest

AI-generates "behavior pills" (concept-level bundles) from a natural language
...
|-----------|-----|------|----------|-------------|
| playbookId | path | string | Yes | Playbook UUID |
| intent | body | string | No | Natural language style description (e.g. "warm, patient, exam-focused") |

**Response** `200`
```json
{ ok, pills: BehaviorPill[], interpretation: string }
```
...
Fetch a single ComposedPrompt by ID — rendered prompt, llmPrompt

**Auth**: OPERATOR · **Scope**: `prompts:read`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| promptId | path | string | Yes | ComposedPrompt UUID |
| siblings | query | number | No | How many sibling prompts (same playbook) to return (default 10) |

**Response** `200`
```json
{ ok: true, prompt, siblings }
```
...
Diff this ComposedPrompt against another (by query param `against`).

**Auth**: OPERATOR · **Scope**: `prompts:read`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| promptId | path | string | Yes | ComposedPrompt UUID (the "right" side of the diff) |
| against | query | string | No | Either another prompt UUID, or the keyword "previous" (default). |

**Response** `200`
```json
{ ok: true, left, right, unifiedDiff, lines }
```
...
**Response** `200`
```json
{ ok, score, totalQuestions, correctCount, uplift? }
```
...
**Response** `200`
```json
{ ok, questions: SurveyStepConfig[], questionIds: string[], skipped: boolean, skipReason?: string }
```
...
Resolves the student's next stop via a 4-state machine (continuous mode):
...
Mark onboarding as complete for a caller.
...
Create a new subject with optional qualification metadata.

**Auth**: Session · **Scope**: `subjects:create`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| slug | body | string | No | Unique slug (required) |
| name | body | string | No | Display name (required) |
| description | body | string | No | Subject description |
| defaultTrustLevel | body | string | No | Default trust level (default: UNVERIFIED) |
...
**Auth**: Session · **Scope**: `subjects:delete`
...
Get subject detail with sources, domains, and latest curriculum.
...
Update subject fields (name, description, trust level, qualification info, active status).

**Auth**: Session · **Scope**: `subjects:update`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| name | body | string | No | Display name |
| description | body | string | No | Subject description |
| defaultTrustLevel | body | string | No | Default trust level for sources |
| qualificationBody | body | string | No | Awarding body |
| qualificationRef | body | string | No | Qualification reference |
| qualificationLevel | body | string | No | Qualification level |
| isActive | body | boolean | No | Active status |
...
Unlink a domain from a subject.

**Auth**: Session · **Scope**: `subjects:update`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| domainId | body | string | No | Domain ID to unlink (required) |
...
List domains linked to a subject.
...
Link a domain to a subject.

**Auth**: Session · **Scope**: `subjects:update`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| domainId | body | string | No | Domain ID to link (required) |
...
Detach a source from a subject (does not delete the ContentSource itself).

**Auth**: Session · **Scope**: `subjects:update`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| sourceId | body | string | No | Source ID to detach (required) |
...
List content sources attached to a subject with assertion counts.
...
### `PATCH` /api/subjects/:subjectId/sources

Update a SubjectSource's tags (e.g. toggle student-material visibility).

**Auth**: Session · **Scope**: `subjects:update`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| sourceId | body | string | No | Source ID to update (required) |

**Response** `200`
```json
{ ok: true, subjectSource: {...} }
```
...
Attach an existing source or create a new one and attach it to a subject.

**Auth**: Session · **Scope**: `subjects:update`

| Parameter | In | Type | Required | Description |
|-----------|-----|------|----------|-------------|
| sourceId | body | string | No | Existing source ID to attach |
| slug | body | string | No | Slug for new source (required if no sourceId) |
| name | body | string | No | Name for new source (required if no sourceId) |
| trustLevel | body | string | No | Trust level override |
| sortOrder | body | number | No | Sort order (default: 0) |
...
Returns comprehensive system readiness status for the analyze workflow. Checks prerequisites (specs, parameters, run configs), data sources (callers, calls, transcripts), and suggests next actions. Result cached 5s in-memory per Cloud Run instance.
...
Returns lightweight task counts by status for the current user.

**Auth**: Session · **Scope**: `tasks:read`

**Response** `200`
```json
{ ok: true, counts: { processing: number, completedRecent: number } }
```
...
Generate a short TTS audio clip for the voice sample

**Auth**: session OPERATOR · **Scope**: `voice:sample:create`

**Response** `200`
```json
audio/mpeg binary stream (Content-Type:audio/mpeg)
```
...
### `GET` /api/voice/[slug]/catalog

Returns the catalog of legal voiceIds per TTS engine routed

**Auth**: session OPERATOR · **Scope**: `voice:catalog:read`
...
**Response** `501`
```json
{ ok: false, error: "Provider does not expose getVoiceCatalog" }
```
...
Load wizard step definitions from a spec. Accepts either `wizard` (name resolved via config) or `slug` (direct). Returns 404 if spec not found.
...
**Response** `200`
```json
{ ok: true, steps: WizardStep[], source: "database" }
```
