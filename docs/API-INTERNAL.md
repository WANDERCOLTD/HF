...
Fetch pre-, mid-, and post-survey answers for a caller (stored as CallerAttribute records)
...
**Response** `200`
```json
{ ok: true, pre: Record, mid: Record, post: Record }
```
...
**Auth**: session (VIEWER+) · **Scope**: `student:write`
...
**Auth**: session (VIEWER+) · **Scope**: `student:read`
