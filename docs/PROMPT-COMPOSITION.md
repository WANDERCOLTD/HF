...
| `preamble.ts` | `computePreamble` | `_preamble` | `_assembled` (identity only). **#790 (S8)** `criticalRules` short-circuits on `Playbook.config.firstCallMode === 'baseline_assessment'` (call 1) to inject `BASELINE_ASSESSMENT_RULE` (override path: `prompt_preamble.config.criticalRules.baselineAssessment`). `teach_immediately` requires no preamble change — the existing branches already inject `returningCallerByMode[teachingMode]` regardless of `isFirstCall`. Default `onboarding` preserves byte-identical pre-#790 output. **#1956 (Boaz/Eldar gap analysis Unit 1.3)** — when the locked module's `AuthoredModuleSettings.silentMode === true`, the announced `BASELINE_ASSESSMENT_RULE` is replaced by `BASELINE_ASSESSMENT_RULE_SILENT` (override path: `prompt_preamble.config.criticalRules.baselineAssessmentSilent`). Diagnostic-only structural directives preserved; "BASELINE_ASSESSMENT:" announcement label + "first call captures diagnostic evidence" framing dropped; tutor explicitly instructed not to signal phase boundaries. Orthogonal-knobs invariant: `firstCallMode` controls structure, `silentMode` controls announcement wording. Module-scoped — requires `sharedState.lockedModule` set; non-locked sessions and non-matching modules fall through to the announced default. |
