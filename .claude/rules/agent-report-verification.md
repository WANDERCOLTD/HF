...
| **[`scripts/gh-pr-create.sh`](../../scripts/gh-pr-create.sh)** `verify_no_unverified_negatives()` (commit-time gate, born 2026-06-15) | Scans the PR body for negative-shaped phrases (`doesn't exist`, `no callers`, `dead code`, `not wired`, `is not implemented`, …) and rejects the PR if any negative line lacks an inverse-probe marker within ±1 line. Acceptable markers: file:line citation, `[verified]`, `[probed]`, `[inverse-probe:…]`, `[unverified]` (explicit demote), `[skip-claim-check]` (per-line escape). PR-wide bypass: `--no-agent-claim-check`. | Turning convention into a structural gate at the PR boundary. Pinned by 8-case vitest at `apps/admin/tests/scripts/gh-pr-create-agent-claim.test.ts`. |
