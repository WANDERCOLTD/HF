...
3. **All concurrent sessions MUST use worktrees.** Harness startup (#904) blocks shared-tree sessions when peers exist — `session-start.sh` exits 2 with a `git worktree add ../HF-myrole feat/your-branch && cd ../HF-myrole` instruction before the session reaches its first tool call. Per-session opt-out: `HF_FORCE_SHARED_TREE=1 claude` (parallels per-command `HF_FORCE_GIT=1`). The PreToolUse `git-lock-enforcer.sh` from #849 remains as defence-in-depth for anything that slips past startup. When you spawn peer agents that touch code, use `isolation: "worktree"` on the `Agent` tool — the agent gets its own `git worktree add ../HF-feat-X feat/X-…` checkout, lock-keyed on `git rev-parse --show-toplevel`, so every worktree is its own PRIMARY slot.
