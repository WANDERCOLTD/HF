    ...
    playbooks: [
      {
        ...
        isActive: true,
        isLatest: true,
        config: { modulesAuthored: false } as unknown as never,
      } as unknown as never,
    ],
    systemSpecs: [],
    onboardingSpec: null,
    subjectSources: mayaSubjectSources as unknown as never,
    ...
    it("when requestedModuleId is set, moduleToReview ignores recentCalls-based heuristic", async () => {
      ...
      const data = makeMayaLoadedData({
        ...
        recentCalls: [
          ...
          { id: "c-c", endedAt: new Date() } as unknown as never,
          { id: "c-d", endedAt: new Date() } as unknown as never,
          { id: "c-e", endedAt: new Date() } as unknown as never,
          { id: "c-f", endedAt: new Date() } as unknown as never,
          { id: "c-g", endedAt: new Date() } as unknown as never,
          { id: "c-h", endedAt: new Date() } as unknown as never,
        ],
      });
      const result = await computeSharedState(data, makeResolvedSpecs(), {}, undefined, "part2");
      const reviewKey = result.moduleToReview?.slug ?? result.moduleToReview?.id;
