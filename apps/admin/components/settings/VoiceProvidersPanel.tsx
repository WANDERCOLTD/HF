...
/**
 * Voice Providers CRUD body (AnyVoice #1031).
 *
 * Shared between the standalone page (`/x/settings/voice-providers`)
 * and the unified Settings panel (`/x/settings#voice_providers`).
 * Credentials always masked — the API never returns raw values.
 */
...
interface Props {
  /** Page-style heading + subtitle. Hidden when embedded as a settings panel
   *  (the panel layout supplies its own title). */
  showHeading?: boolean;
}

export function VoiceProvidersPanel({ showHeading = false }: Props = {}) {
    ...
    <div>
      {showHeading ? (
        ...
        <p className="hf-section-desc">
          Manage voice-call providers. Credentials are masked; the default provider serves any
          caller without a per-caller override.
        </p>
      )}
