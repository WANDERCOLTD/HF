...
async function main() {
  ...
  // lives on the parent Session row. This sim-drive script doesn't go
  // through `createSession` (it's a CLI operator tool, not the live
  // sim API route), so the call lands without a Session parent — the
  // composer's `nextLearnerFacingNumber` loader treats it as a non-
  // qualifying historical row.
