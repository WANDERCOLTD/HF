 ...
 *
 *   Goes through `updatePlaybookConfig()` (#820) which enforces the
 *   TUNER -> COMPOSE chain-contract (Link 3 sub-contract): any change
 *   to a COMPOSE-affecting namespace fans out recompose-all across
 *   the active roster automatically.
 ...
 * @response 200 { ok: true, recomposed: boolean, changedKeys: string[] }
  ...
  try {
    ...
    try {
      const result = await updatePlaybookConfig(
        courseId,
        (pbConfig) => {
          if (body.welcome) pbConfig.welcome = body.welcome;
          if (body.nps) pbConfig.nps = body.nps;

          // #417 Story C — banding override. `null` clears (fall back to
          // the SKILL_MEASURE_V1 contract defaults).
          if (body.skillTierMapping !== undefined) {
            if (body.skillTierMapping === null) delete pbConfig.skillTierMapping;
            else pbConfig.skillTierMapping = body.skillTierMapping;
          }
          if (body.skillScoringEmaHalfLifeDays !== undefined) {
            if (body.skillScoringEmaHalfLifeDays === null) delete pbConfig.skillScoringEmaHalfLifeDays;
            else pbConfig.skillScoringEmaHalfLifeDays = body.skillScoringEmaHalfLifeDays;
          }
          if (body.skillMinCallsToFull !== undefined) {
            if (body.skillMinCallsToFull === null) delete pbConfig.skillMinCallsToFull;
            else pbConfig.skillMinCallsToFull = body.skillMinCallsToFull;
          }

          // #779 — Felt Progress progressNarrative. Pass `null` to clear.
          if (body.progressNarrative !== undefined) {
            if (body.progressNarrative === null) delete pbConfig.progressNarrative;
            else pbConfig.progressNarrative = body.progressNarrative;
          }

          // #780 — Felt Progress offboardingSummary. Same null-clears semantics.
          if (body.offboardingSummary !== undefined) {
            if (body.offboardingSummary === null) delete pbConfig.offboardingSummary;
            else pbConfig.offboardingSummary = body.offboardingSummary;
          }
          ...
          if (body.firstCallMode !== undefined) {
            if (body.firstCallMode === null) delete pbConfig.firstCallMode;
            else pbConfig.firstCallMode = body.firstCallMode;
          }

          // surveys.pre.enabled is now COMPUTED-ONLY from welcome.* (see
          // isPreSurveyEnabled); do NOT write it. surveys.post.enabled has no
          // welcome-side mirror yet, so mirror from nps.enabled when nps saves.
          if (body.nps) {
      ...
      return NextResponse.json({
        ok: true,
        recomposed: result.composeAffected,
        changedKeys: result.changedKeys,
      });
    } catch (e) {
      // updatePlaybookConfig throws "playbook ... not found" — translate to 404.
